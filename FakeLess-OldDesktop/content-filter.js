(function() {
    ...
    // Settings-based blocking control
    let adultBlockingEnabled = true;
    let wordFilteringEnabled = true;

    // Run blocking check immediately if enabled
    function initBlocking() {
        if (adultBlockingEnabled && blockIfAdultContent()) {
            return true;
        }
        return false;
    }
    ...
    browser.runtime.onMessage.addListener(function(message, sender, sendResponse) {
        if (message.action === 'updateSettings') {
            ...
            if (message.settings) {
                adultBlockingEnabled = message.settings.enableAdultBlock !== false;
                wordFilteringEnabled = message.settings.enableFilter !== false;
            }
    ...
    browser.storage.sync.get({
        ...
        adultBlockingEnabled = settings.enableAdultBlock !== false;
        wordFilteringEnabled = settings.enableFilter !== false;
        
        // Run adult blocking check first
        if (initBlocking()) {
            return; // Page was blocked, stop here
        }
        
        // Initialize word filtering if enabled
        if (wordFilteringEnabled && document.body) {
            initializeFilter();
        }
    });
