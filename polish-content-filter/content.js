(function() {
    ...
    browser.runtime.onMessage.addListener(function(message, sender, sendResponse) {
        if (message.action === 'updateSettings') {
            // Update deepfake detector settings if available
            if (window.deepfakeDetector) {
                window.deepfakeDetector.updateSettings(message.settings);
            }
        }
    });

    // Load settings and initialize deepfake detector
    browser.storage.sync.get({
        ...
        if (window.deepfakeDetector) {
            window.deepfakeDetector.updateSettings(settings);
        }
    });
